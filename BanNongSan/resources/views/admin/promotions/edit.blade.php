...
<body>
        ...
        ***********************************-->
        <div class="content-body">
            ...
            <div class="container-fluid">
                ...
                <form action="{{ route('admin.promotions.update', $promotion->ma_khuyen_mai) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="form-group">
                        <label for="ten_khuyen_mai">Tên khuyến mãi:</label>
                        <input type="text" name="ten_khuyen_mai" id="ten_khuyen_mai" class="form-control" value="{{ old('ten_khuyen_mai', $promotion->ten_khuyen_mai) }}" required>
                    </div>
            
                    <div class="form-group">
                        <label for="ngay_bat_dau">Ngày bắt đầu:</label>
                        <input type="date" name="ngay_bat_dau" id="ngay_bat_dau" class="form-control" value="{{ old('ngay_bat_dau', $promotion->ngay_bat_dau) }}" required>
                    </div>
            
                    <div class="form-group">
                        <label for="ngay_ket_thuc">Ngày kết thúc:</label>
                        <input type="date" name="ngay_ket_thuc" id="ngay_ket_thuc" class="form-control" value="{{ old('ngay_ket_thuc', $promotion->ngay_ket_thuc) }}" required>
                    </div>
            
                    <div class="form-group">
                        <label for="mo_ta">Mô tả:</label>
                        <textarea name="mo_ta" id="mo_ta" class="form-control">{{ old('mo_ta', $promotion->mo_ta) }}</textarea>
                    </div>
            
                    <h3>Chọn sản phẩm và giảm giá</h3>
                    <div id="product-discounts">
                        @foreach($promotion->sanPhamKhuyenMai as $index => $item)
                            ...
                            <div class="form-group">
                                ...
                                <select name="san_pham[{{ $index }}][ma_san_pham]" class="form-control">
                                    @foreach($products as $product)
                                        <option value="{{ $product->MaSanPham }}" @if($product->MaSanPham == $item->ma_san_pham) selected @endif>{{ $product->TenSanPham }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="san_pham[{{ $index }}][giam_gia]">Giảm giá (%):</label>
                                <input type="number" name="san_pham[{{ $index }}][giam_gia]" class="form-control" min="0" max="100" value="{{ $item->giam_gia }}" required>
                            </div>
                        </div>
                        @endforeach
                    </div>
    ...
    ***********************************-->
    ...
    <script>
        let index = {{ count($promotion->sanPhamKhuyenMai) }};
